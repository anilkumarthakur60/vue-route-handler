    ...
    <nav class="navbar">
      ...
      <div :class="['offcanvas', { show: isSidebarMenuVisible }]">
        <div class="offcanvas-header">
          <button type="button" class="btn-close" @click="toggleSidebarMenu">
            <i class="fas fa-times"></i>
          </button>
        </div>
        <div class="offcanvas-body">
          <RouterLink v-for="(item, index) in routeLists" :to="item.path" class="navbar-item"
            :class="activeRoute(item.name)" :key="index" @click="toggleSidebarMenu">
            {{ item.label }}
          </RouterLink>
        </div>
      </div>
      <div class="navbar-menu">
        <RouterLink v-for="(item, index) in routeLists" :to="item.path" class="navbar-item"
          :class="activeRoute(item.name)" :key="index">
          {{ item.label }}
        </RouterLink>
      </div>
    </nav>
