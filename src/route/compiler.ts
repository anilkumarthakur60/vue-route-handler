import { filterObject, guardError, trim } from "../utils/common";
...
export class Compiler {
  ...
  compilePath(nested: any) {
    const cleanedPath =
      trim([this.route._prefixClamp, this.route._path].join("/")) || "";
    return nested ? cleanedPath : `/${cleanedPath}`;
  }

  compileName() {
    const separator = Factory.nameSeparator;
    const compiledName = [this.route._nameClamp, this.route._name].join(
      separator
    );
  ...
  guardRejector(context: CompilerContext, next: Function): Function {
    return (rejection: any) => {
      ...
      if (
        context.to.name == rejection.name ||
        context.to.path === rejection.path
      ) {
